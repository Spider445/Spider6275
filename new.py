...
# Join :- @rzrdevil #bgmiddoserpython
...
# Join :- @rzrdevil # File to store command logs
LOG_FILE = "log.txt"


# Join :- @rzrdevil # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @rzrdevil # Add more proxy addresses as needed
]
...
# Join :- @rzrdevil # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @rzrdevil # Check if line is not empty
...
# Join :- @rzrdevil # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @rzrdevil # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @rzrdevil # Function to clear logs
def clear_logs():
    try:
...
# Join :- @rzrdevil # Function to record command logs
...
# Join :- @rzrdevil # Function to handle the reply when free users run the /bgmi command
...
# Join :- @rzrdevil # Handler for /bgmi command
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @rzrdevil # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @rzrdevil # Updated to accept target, time, and port
            target = command[1]
...
# Join :- @rzrdevil # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @rzrdevil # Import the 'time' module for sleep functionality
import time
